...
heading "[Starting] Acceptance Testing using Beaker-rspec"
	pushd $WORKSPACE/coverage	
		  result=`echo sudo kitchen test $WORKSPACE/coverage`
...
heading "[Starting] Unit Testing and Code Coverage Using rspec-puppet tool"
	pushd $WORKSPACE/coverage
		unit_testing_resource_coverage=`echo rspec --format documentation $WORKSPACE/coverage`
...
msg "6. Acceptance testing : "
	PUPPET_ACCEPTANCE_TESTING_MAIL=""
	  ...
	  time_taken=`grep -oP '\Finished in\K[^\(f]+' $WORKSPACE/log/acceptance_testing.txt`
